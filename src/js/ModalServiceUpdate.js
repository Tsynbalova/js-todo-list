...
export class ModalServiceUpdate{
    ...
    _onUpdate(e) {
        ...
        this.api.patching(this._card.cardId, formData).then((data) => {
    ...
    _validateFormUpd(form, formData) {
        ...
        if (!(formData.userId <= 10 && formData.userId >= 1)) {
            errors.push('Поле пользователя должно иметь значение в диопазоне от 1 до 10');
        }
        if (formData.title.length >= 90) {
            errors.push('Поле наименование должно иметь не более 90 символов');
        }
